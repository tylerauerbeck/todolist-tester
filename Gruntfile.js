...
module.exports = function(grunt) {
  ...
  grunt.registerTask("build-image", "Build the image", function(imageId) {
    ...
    const rc = shell.exec(
      "docker build -t todolist:" + imageId + " -f ./dist/Dockerfile ./dist"
    ).code;
    if (rc > 0) {
  ...
  grunt.registerTask(
    "deploy",
    "deploy the node js app to a docker container and start it in the correct mode",
    function(target_env, build_tag) {
      grunt.log.ok(
        "this task must run on a host that has the Docker Daemon running on it"
      );
      const ports = {
        ci: "9001",
        si: "9002",
      ...
      if (target_env === undefined || build_tag === undefined) {
        grunt.fail.warn(
          "Required param not set - use grunt deploy:<target>:<tag>"
        );
      } else {
        const shell = require("shelljs");
        ...
        if (target_env === "ci") {
          ...
          if (isMongo > 0) {
            ...
            shell.exec(
              "docker run --name devops-mongo -p 27017:27017 -d mongo"
            );
          }
          const rc = shell.exec(
            "docker run -t -d --name todolist-" +
              target_env +
              " --link devops-mongo:mongo.server -p " +
              ports[target_env] +
              ":" +
              ports[target_env] +
              " --env NODE_ENV=" +
  ...
  grunt.registerTask("test", function(target, environ) {
    ...
    } else grunt.task.run(["test:server"]);
  });
